...
4. Treat `reference-doc/IN/**/2026.03.JPM.Platform/index.md` plus the matching per-module markdown as the primary module PRD source when present.
   - Use it for module workflow, detailed scope, agent behavior, and terminology.
   - For dashboard work, use the index mapping back to Chapter 1 when there is no standalone dashboard chapter.
   - If extracted wording looks ambiguous, verify against the sibling `2026.03.JPM.Platform.pdf`.
5. Treat `reference-doc/IN/**/gemeni_chat.md` as the secondary scope, naming, and cross-check source.
6. Ignore `reference-doc/PENDING/**` unless the user explicitly confirms approval.
7. Load the module doc at `coding-doc/modules/<module>.md`.
   - If it does not exist yet, start from `coding-doc/modules/_template.md`.
8. Decide the module archetype and reading path before editing code.
...
9. Load the module's current route, component, mock-data, and shared UI dependencies before editing code.
10. If the reference prototype only provides a generic page for that module, expand from the documented module intent and existing shared patterns instead of redesigning the shell.
11. When code changes affect module behavior, layout blocks, mock-data fields, or acceptance expectations, update in the same pass:
   - `coding-doc/modules/<module>.md`
   - any relevant `coding-doc/*.md`
   - module code and `lib/mock-data/*`
12. Validate the module against the module checklist before finishing.
...
- Record concrete source references inside module docs so later edits stay anchored, including the matching split PRD file when one exists.
